...
export const getLocaleTokenLogoURL = memoize(
  (token?: Token) => {
    if (token && mapping[token.chainId]) {
      return `/images/${token.symbol}.png`;
    }
    return null;
  },
  (t) => `${t?.chainId}#${t?.address}`
...
export const getCurrencyLogoUrls = memoize(
  (currency: Currency | undefined, { useTrustWallet = true }: GetLogoUrlsOptions = {}): string[] => {
    ...
    const localeTokenLogo = getLocaleTokenLogoURL(currency?.wrapped);
    const logoUrl = currency ? getTokenListTokenUrl(currency.wrapped) : null;
    return [getCommonCurrencyUrl(currency), useTrustWallet ? trustWalletLogo : undefined,localeTokenLogo, logoUrl].filter(
